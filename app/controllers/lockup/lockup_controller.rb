    ...
    def unlock
      if params[:lockup_codeword].present?
        ...
        unless user_agent.match(/crawl|googlebot|slurp|spider|bingbot|tracker|click|parser|spider/)
          ...
          if @codeword == ENV["LOCKUP_CODEWORD"].to_s.downcase || ENV["lockup_codeword"].to_s.downcase || Figaro.env.lockup_codeword || ((Rails::VERSION::MAJOR >= 4 && Rails::VERSION::MINOR >= 1) && @codeword == Rails.application.secrets.lockup_codeword.to_s.downcase)
            set_cookie
            run_redirect
          end
        else
