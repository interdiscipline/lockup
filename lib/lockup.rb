  ...
  def check_for_lockup
    if ENV["LOCKUP_CODEWORD"].present? || ENV["lockup_codeword"].present? || ((Rails::VERSION::MAJOR >= 4 && Rails::VERSION::MINOR >= 1) && Rails.application.secrets.lockup_codeword.present?)
      if cookies[:lockup].present?
        if cookies[:lockup] == ENV["LOCKUP_CODEWORD"].to_s.downcase || cookies[:lockup] == ENV["lockup_codeword"].to_s.downcase || ((Rails::VERSION::MAJOR >= 4 && Rails::VERSION::MINOR >= 1) && cookies[:lockup] == Rails.application.secrets.lockup_codeword.to_s.downcase)
          return
        else
