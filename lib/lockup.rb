...
module Lockup
  ...
  def check_for_lockup
    if ENV["LOCKUP_CODEWORD"].present? || Rails.application.secrets.lockup_codeword.present?
      if cookies[:lockup].present?
        if cookies[:lockup] == ENV["LOCKUP_CODEWORD"].to_s.downcase || cookies[:lockup] == Rails.application.secrets.lockup_codeword.to_s.downcase
          return
        else
